...
def page_history(links, ctx):
    ...
    # The z-score and the drift to the closing line were on this page once, in
    # the tables and the figures above them. Both are true, and both are for
    # whoever is working on the model rather than for a reader: `fb.py history`
    # prints them. What a reader gets is the record, what it claimed, and the
    # chart.
...
<section class="card">
  ...
  {_record_table("Band", [(BAND_LABEL.get(row["band"], row["band"]), row)
                          for row in bands])}
</section>"""
...
# On a phone: the band or market and what it did, its record and what it
# said, then what is pending in small print.
BAND_ROLES = ["title", "sub", "end", "end2 label", "meta label"]


def _record_table(first, rows):
    """A record split one way: (name, summary) per row, as `ledger.summary`."""
    return c.table(
        [first, "Record", "Did", "Said", "Pending"],
        [[c.e(name), f"{row['wins']}&ndash;{row['losses']}", _pct(row["hit_rate"]),
          _pct(row["expected"]), str(row["pending"])] for name, row in rows],
        numeric_from=1, raw=True, blank=NONE, roles=BAND_ROLES)
...
<section class="card">
  ...
  that has gone wrong on its own can hide inside a band's pooled record.</p>
  {_record_table("Market", [(GROUPS.get(row["group"], row["group"]), row)
                            for row in rows])}
</section>"""
...
def _history_verdict(head):
    ...
    count = (f"{head['wins']} landed against {expected:.1f} claimed, "
             f"over {settled} settled pick(s)")
    if settled < 100:
        return c.status_block(
            "warning", "Far too early to read anything into this",
            f"{count}. At this size an honest forecast and a dishonest one land "
            "in much the same place. A hit rate starts meaning something in the "
    ...
    return c.status_block(
        ...
        f"{count}. Falling this far short is something an honest forecast does "
        "about one time in forty, and worth finding the cause of.")
